...
void friccion_aislada_esferica_estatica_86e574d6_4_resetStateVector ( const
void * mech , double * state ) { double xx [ 2 ] ; ( void ) mech ; xx [ 0 ] =
1.0 ; xx [ 1 ] = 0.0 ; state [ 0 ] = xx [ 0 ] ; state [ 1 ] = xx [ 1 ] ;
state [ 2 ] = xx [ 1 ] ; state [ 3 ] = xx [ 1 ] ; state [ 4 ] = xx [ 1 ] ;
state [ 5 ] = xx [ 1 ] ; state [ 6 ] = xx [ 1 ] ; state [ 7 ] = xx [ 0 ] ;
state [ 8 ] = xx [ 1 ] ; state [ 9 ] = xx [ 1 ] ; state [ 10 ] = xx [ 1 ] ;
state [ 11 ] = xx [ 1 ] ; state [ 12 ] = xx [ 1 ] ; state [ 13 ] = xx [ 1 ] ;
} PmfMessageId
...
mech , const double * dofDeltas , double * state ) { double xx [ 28 ] ; (
void ) mech ; xx [ 0 ] = state [ 0 ] ; xx [ 1 ] = state [ 1 ] ; xx [ 2 ] =
state [ 2 ] ; xx [ 3 ] = state [ 3 ] ; xx [ 4 ] = dofDeltas [ 0 ] ; xx [ 5 ]
...
mech , size_t stageIdx , size_t primIdx , double mag , bool doPerturbVelocity
...
perturbState_0_0 ( mag , state ) ; break ; case 1 : perturbState_0_0v ( mag ,
state ) ; break ; case 12 : perturbState_1_0 ( mag , state ) ; break ; case
13 : perturbState_1_0v ( mag , state ) ; break ; } } void
friccion_aislada_esferica_estatica_86e574d6_4_propagateMotion ( const void *
mech , const double * state , double * motionData ) { double xx [ 33 ] ; (
void ) mech ; xx [ 0 ] = 0.6123724356957946 ; xx [ 2 ] = - 0.3535533905932738
; xx [ 3 ] = xx [ 0 ] ; xx [ 4 ] = - xx [ 0 ] ; xx [ 5 ] = xx [ 2 ] ; xx [ 6
] = xx [ 2 ] ; xx [ 0 ] = 6.658676150629137e-9 ; xx [ 7 ] = - ( state [ 0 ] +
xx [ 0 ] * state [ 2 ] ) ; xx [ 8 ] = - ( state [ 1 ] + xx [ 0 ] * state [ 3
...
void * mech , size_t constraintIdx , const double * state , const double *
motionData , double * error ) { ( void ) mech ; ( void ) state ; ( void )
motionData ; ( void ) error ; ( void ) state ; switch ( constraintIdx ) { }
return 0 ; } size_t
...
const void * mech , const double * state , const double * motionData , double
* J ) { ( void ) mech ; ( void ) state ; ( void ) motionData ; ( void ) J ;
return 0 ; } PmfMessageId
friccion_aislada_esferica_estatica_86e574d6_4_convertStateVector ( const void
* asmMech , const void * simMech , const double * asmState , double *
simState , void * neDiagMgr0 ) { NeuDiagnosticManager * neDiagMgr = (
